...
const uploadsDir = (0, upload_1.getUploadsDir)();
app.use("/uploads", (req, res, next) => {
    res.header("Access-Control-Allow-Origin", "*");
    res.header("Cross-Origin-Resource-Policy", "cross-origin");
    next();
}, express_1.default.static(uploadsDir), (_req, res) => {
...
async function syncDatabaseSeed() {
    ...
    `);
        const studentRes = await database_1.default.query("SELECT id, email, name FROM users WHERE role = 'student' ORDER BY id ASC");
        const staffRes = await database_1.default.query("SELECT id, email, name FROM users WHERE email = 'j.folakemi@university.edu' OR role = 'staff' ORDER BY id ASC");
        const officerRes = await database_1.default.query("SELECT id, email, name FROM users WHERE role = 'officer' ORDER BY id ASC");
