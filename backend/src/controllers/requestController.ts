...
export async function getAllRequests(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (user.role === "student" || user.role === "staff") {
      conditions.push(
        `(sr.submitted_by = $${idx} OR LOWER(u.email) = LOWER($${idx + 1}) OR LOWER(u.name) = LOWER($${idx + 2}) OR (LOWER($${idx + 1}) LIKE '%folakemi%' AND (LOWER(u.email) LIKE '%folakemi%' OR LOWER(u.name) LIKE '%janet%' OR LOWER(u.name) LIKE '%newest%')))`
      );
